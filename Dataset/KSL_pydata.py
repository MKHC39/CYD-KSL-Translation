...
class KSLFeeder(Dataset):
    ...
    def __init__(
        ...
        split_mod_dev: Sequence[int] = (8,),
        split_mod_test: Sequence[int] = (9,),
        # Misc
        ...
        **_: Any,
        ...
        if split == "by_word_mod":
            k = int(split_mod_k)
            if self.mode == "train":
                allow = set(int(x) for x in split_mod_train)
            elif self.mode in ("dev", "val", "valid", "validation"):
                allow = set(int(x) for x in split_mod_dev)
            else:
                allow = set(int(x) for x in split_mod_test)
            self.samples = [s for s in all_samples if (s.w % k) in allow]
        else:
